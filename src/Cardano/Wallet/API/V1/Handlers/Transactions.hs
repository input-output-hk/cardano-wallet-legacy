...
-- | Given a 'Payment' as input, tries to generate a new 'Transaction',
-- submitting it to the network eventually.
newTransaction
...
newTransaction aw payment@Payment{..} = liftIO $ do
    ...
    let inputGrouping =
            toInputGrouping
            $ fromMaybe
                (WalletInputSelectionPolicy defaultInputSelectionPolicy)
                pmtGroupingPolicy
    res <- WalletLayer.pay aw
        (maybe mempty coerce pmtSpendingPassword)
        inputGrouping
        SenderPaysFee
        payment
    ...
    case res of
        Left err ->
            throwM err
        Right (_, meta) ->
            txFromMeta aw NewPaymentUnknownAccountId meta
...
getTransactionsHistory pw mwalletId mAccIdx mAddr requestParams fops sops =
    liftIO $ do
        mRes <- WalletLayer.getTransactions pw
                mwalletId mAccIdx mAddr requestParams fops sops
        case mRes of
            Left err  -> throwM err
            Right res -> confirmIfCurrentlyRestoring pw res

confirmIfCurrentlyRestoring
    :: PassiveWalletLayer IO
    -> APIResponse [Transaction]
    -> IO (APIResponse [Transaction])
confirmIfCurrentlyRestoring _ resp = do
    mrestInfo <- Internal.lookupRestorationInfo pw hdRootId
    case mrestInfo of
        Just restInfo | shouldWeConfirm restInfo ->
            pure (fmap (fmap markConfirmed) resp)
        _ ->
            pure resp
  where
    pw = error "PassiveWallet"
    hdRootId = error "how to get an HdRootId from request params?"
    markConfirmed = error "mark it confirmed friend"
    shouldWeConfirm :: Internal.WalletRestorationInfo -> Bool
    shouldWeConfirm = error "inspect the restoration info, return if safe to confirm"
...
estimateFees aw payment@Payment{..} = liftIO $ do
    let inputGrouping =
            toInputGrouping
            $ fromMaybe
                (WalletInputSelectionPolicy defaultInputSelectionPolicy)
                pmtGroupingPolicy
    res <- WalletLayer.estimateFees aw inputGrouping SenderPaysFee payment
    case res of
        Left err  -> throwM err
