...
launchNode c st topology ns = run exe args
  where
    ...
    walletArgs' = [ "--tlscert", tt (st </> "tls" </> "server" </> "server.crt")
                  ...
                  , "--node-tls-client-cert", tt (st </> "tls" </> "client" </> "client.crt")
                  , "--node-tls-key", tt (st </> "tls" </> "client" </> "client.key")
                  , "--node-tls-ca-cert", tt (st </> "tls" </> "client" </> "ca.crt")
                  ]
...
programName Wallet = "cardano-wallet-server"
